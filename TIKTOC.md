...
**Central thesis:** Designers design in Figma — that is not changing. The Figma file is the source of truth for visual decisions. The problem is getting those decisions out reliably, keeping them in sync with production, and making them machine-readable enough that CLIs, build tools, and AI agents can consume them without a human in the loop. This book is about the extraction layer: the audit layer, the token pipeline, the asset export automation, the MCP workflows, and the file discipline required for a CLI to build from the canvas.

**The gap this fills:** No existing practitioner resource addresses the full extraction stack end-to-end: naming discipline, programmatic audit, token pipeline, asset automation, machine-readable component specs, MCP integration, CLI workflows, and what it means for a Figma file to be structured well enough that a command-line tool can consume it safely. Existing resources either cover the Figma UI (for designers) or individual API endpoints (for developers). Nothing bridges them from the perspective of making a real file production-ready.
...
The book should make the command line visible as the practical operating surface of the extraction layer. The reader is not merely "calling the API"; they are building a small, testable toolkit that can be run locally, in CI, and by AI coding agents under explicit governance.

**Core CLI artifacts introduced across the book:**

- `figma-ping.js` — verifies token, file key, plan/seat access, endpoint reachability, and rate-limit headers before any serious work begins.
- `figma-read.mjs` — fetches the file graph, normalizes the response, writes a stable local fixture, and prints the file/page/component inventory.
- `figma-audit.js` — checks naming, token hygiene, component hygiene, brand compliance, accessibility risks, and structural completeness.
- `figma-fix-plugin/` — staged Plugin API remediation for bulk renames and metadata fixes, with human approval before writes.
- `extract-tokens.mjs` — extracts variables or imports Tokens Studio output, resolves aliases, validates modes, and writes DTCG-compatible JSON.
- `validate-tokens.mjs` — fails fast on broken aliases, malformed values, missing modes, and platform-incompatible names.
- `export-assets.mjs` — batches image endpoint requests, downloads expiring image URLs, post-processes SVG with SVGO, and writes deterministic asset paths.
- `sync-docs.mjs` — generates component inventory, variant tables, and missing-documentation reports for Storybook, Zeroheight, Supernova, or custom portals.
- `monitor-brand.mjs` — produces diffable compliance reports for color, type, spacing, contrast, and token usage.
- `build-spec.mjs` — emits a complete machine-readable component specification JSON for downstream CLIs and code generators.
- `figma-mcp-check.md` / `FIGMA.md` — governs what an AI coding agent may read, infer, generate, or refuse during MCP-backed sessions.

**Stable CLI contract taught throughout:**

1. Environment: `FIGMA_TOKEN`, `FIGMA_FILE_KEY`, optional team/project/library identifiers, and no hardcoded secrets.
2. Input: a file key or node list, plus a declared consumer: human report, token compiler, asset pipeline, documentation site, code generator, or AI agent.
3. Output: both human-readable markdown and machine-readable JSON where downstream automation is expected.
4. Validation: schema checks, rate-limit handling, retry/backoff rules, fixture-based tests, and explicit failure codes for CI.
5. Governance: every command states what it reads, what it writes, what it can safely automate, and what requires human review.

**Terminal outcome:** by the end of the book, the reader can run a local command such as `npm run figma:audit`, `npm run figma:tokens`, `npm run figma:assets`, or `npm run figma:spec`, understand the output, and decide whether the design file is safe for production automation.
...
- The CLI environment contract: `FIGMA_TOKEN`, `FIGMA_FILE_KEY`, scopes, secrets, and local `.env` hygiene
- Failure-first endpoint checks: auth failure, missing file permission, wrong plan, exhausted rate limit, unsupported endpoint

`figma-ping.js` introduced here as the session health check.

Decision rules: which API surface to use for which task.

CLI artifact: `figma-ping.js`, which prints endpoint reachability, plan/rate-limit metadata where available, and a clear next action for each failure.
...
- Local fixtures: saving a redacted file response so CLI code can be tested without hammering the API
- Output shape: component inventory JSON, page/frame report, and missing-metadata report

Worked example: reading a real design system file with `figma-read.mjs`, writing a local fixture, and extracting a component inventory from the raw JSON.
...
- CI behavior: exit codes, baseline snapshots, and when a warning becomes a blocking error
- CLI ergonomics: grouped findings, object links, page names, node IDs, and suggested remediation commands
...
- Machine-readiness as a CLI preflight: commands that pass before token extraction, asset export, documentation sync, or MCP use
- Readiness output: a score is optional; a blocking/non-blocking finding list is required
...
- CLI inputs and outputs: file key, optional collection filters, DTCG-compatible JSON, platform build artifacts, and validation reports
- Fixture tests: known variable set in, expected token JSON out
...
- CLI manifest: mapping node IDs to deterministic repository paths, formats, and optimization rules
- Asset integrity checks: missing node, changed dimensions, empty render, duplicate filename, and changed SVG diff
...
- CLI-generated documentation fragments: component inventory JSON, variant markdown tables, missing-description reports, and stale-doc diffs
...
- `build-spec.mjs`: emitting a schema-validated component specification for a downstream CLI or code generator
- Contract tests: the code generator should fail if the spec omits required fields or contains unresolved design references
...
- CLI-to-agent handoff: passing `figma-audit`, `build-spec`, and token outputs into Claude Code as verified context
- Agent refusal rules: what the coding agent must not infer when Figma data is missing or ambiguous
...
- The production CLI: `figma:ping`, `figma:read`, `figma:audit`, `figma:tokens`, `figma:assets`, `figma:docs`, `figma:brand`, `figma:spec`, and `figma:mcp-check`
- CI/CD wiring: local command, pull request check, scheduled audit, library publish trigger, and human approval gate
- Final capstone: a design system repo where the Figma file can drive tokens, assets, documentation, compliance reports, and AI-agent implementation context
