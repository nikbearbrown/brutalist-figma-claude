...
Produce a standalone D3 v7 HTML file visualizing the MCP architecture for a Figma design system. Chart type: flow diagram with labeled nodes and directed edges. Two parallel tracks feed the AI coding agent: the live MCP track (AI Coding Agent ← MCP Protocol ← Figma MCP Server ← Figma REST API ← Figma Design File) and the CLI context package track (audit.json, spec.json, tokens.json feeding directly to the agent). The Figma Design File node is the shared source for both tracks; color it with `--color-red` as the primary data category. CLI context package nodes use `--color-fill` background. All other nodes use `--color-fill`. Directed edges use `--color-ink` arrows; the CLI extraction arrow from design file to CLI artifacts uses `--color-brown` dashed. Add a "Both paths depend on file quality" annotation box. Layout: three-column arrangement, left column MCP track, center column agent, right column CLI artifacts. Section labels in 10px `--color-secondary` all-caps. Fonts: `'Real Head Pro','FF Real',Lato,sans-serif`. Deliverable: single HTML file, inline CSS, D3 v7 CDN, ResizeObserver redraw, dark-mode `:root` block, `prefers-reduced-motion` suppression, `role="img"` + `aria-labelledby` + `<title>` + `<desc>` on SVG.

> Reference implementation: `d3/13-the-figma-mcp-server-fig-01.html`

### Figure 13.2 — Agent decision tree: component without Code Connect

Produce a standalone D3 v7 HTML file visualizing a three-branch decision tree for an AI coding agent encountering a component without a Code Connect mapping. Chart type: top-down flow diagram with diamond decision nodes and rectangular outcome nodes. Root node: "Agent receives component request via MCP context." Decision 1 (diamond): "Code Connect mapping published?" — YES branch (left) leads to "Generate" outcome in `--color-fill`; NO branch (down) leads to Decision 2. Decision 2 (diamond): "Description field clear and complete?" — YES branch (right) leads to "Escalate to human" outcome filled with `--color-red` (this is the human approval gate — must be visually prominent); NO branch (down) leads to "Refuse and explain" outcome with `--color-ink` border. Add a FIGMA.md annotation box connected by a dashed line to Decision 2, explaining that the governance file governs each branch. Caption: "The governance file determines which branch the agent takes. Without it, the agent chooses for itself." Fonts: `'Real Head Pro','FF Real',Lato,sans-serif`. Deliverable: single HTML file, inline CSS, D3 v7 CDN, ResizeObserver redraw, dark-mode `:root` block, `prefers-reduced-motion` suppression, `role="img"` + `aria-labelledby` + `<title>` + `<desc>` on SVG.

> Reference implementation: `d3/13-the-figma-mcp-server-fig-02.html`
